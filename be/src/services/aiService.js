...
import {
  fetchImageUrlToBuffer,
  optimizeImageBufferForVision,
  jpegBufferToDataUrl,
} from "./visionImageOptimize.js";
...
async function prepareImageForAPI(imageUrlOrPath) {
  ...
  if (!config.openai.visionOptimizeImages) {
  ...
  if (isUrl(imageUrlOrPath)) {
    ...
    try {
      const buf = await fetchImageUrlToBuffer(imageUrlOrPath);
      const jpeg = await optimizeImageBufferForVision(buf);
      return jpegBufferToDataUrl(jpeg);
    } catch (error) {
      console.warn(
        `Vision image optimize failed for URL, using original URL: ${imageUrlOrPath}`,
        error.message,
      );
      return imageUrlOrPath;
    }
  ...
  try {
    const buf = await readFile(imageUrlOrPath);
    try {
      const jpeg = await optimizeImageBufferForVision(buf);
      return jpegBufferToDataUrl(jpeg);
    } catch (optErr) {
      console.warn(
        `Vision image optimize failed for file, using raw base64: ${imageUrlOrPath}`,
        optErr.message,
      );
      return await imageToBase64(imageUrlOrPath);
    }
  } catch (error) {
    console.error(`Failed to read image file: ${imageUrlOrPath}`, error);
    return null;
  }
}
...
BẮT BUỘC SỐ LƯỢNG: "similar" phải có ĐÚNG 2 phần tử; "remedial" phải có ĐÚNG 1 phần tử — không thêm, không bớt.
...
Quy tắc:
1. Cả 2 bài trong **"similar"** PHẢI cùng **loại nhiệm vụ** với bài gốc (lập PT thì similar cũng là lập PT; giải PT thì similar cũng là giải PT; không đổi sang dạng khác).
2. **Nếu bài gốc là LẬP phương trình bậc nhất một ẩn từ lời văn:**
   - Mỗi "problem" PHẢI là một **bài toán lời văn mới** (bối cảnh khác nhau giữa 2 bài: tuổi, chuyển động, mua bán, tỉ lệ, hình học đơn giản…), đủ dữ kiện để học sinh **tự lập** phương trình.
...
1. Đa dạng TRONG CÙNG một nhóm ("similar" 2 bài, "remedial" 1 bài):
   - 2 bài trong "similar" PHẢI thuộc 2 DẠNG CON KHÁC NHAU của cùng chủ đề với bài gốc.
     Ví dụ nếu bài gốc là "phân tích đa thức thành nhân tử":
       * Bài 1: Hằng đẳng thức bình phương ($a^2 \\pm 2ab + b^2$)
       * Bài 2: Hiệu hai bình phương ($a^2 - b^2$) HOẶC tam thức bậc hai tách hạng tử / nhóm hạng tử — chọn 2 dạng khác nhau, không trùng khuôn.
     Ví dụ nếu bài gốc là **lập phương trình bậc nhất một ẩn từ bài toán có lời văn**:
       * Hai bài similar là **hai bài lời văn khác nhau** (tuổi, chuyển động đều, mua bán, tỉ lệ, hình học đơn giản…), mỗi bài yêu cầu **lập** phương trình (có thể thêm "tìm ẩn" nếu đề gốc có).
       * TUYỆT ĐỐI KHÔNG lấy phương trình dạng "$ax+b=c$" sẵn để "giải" thay cho bài lời văn lập PT.
     TUYỆT ĐỐI KHÔNG được sinh 2 bài cùng một khuôn (ví dụ cả 2 bài đều là "bình phương một tổng/hiệu").
   - 1 bài trong "remedial" PHẢI bám SÁT lỗi cụ thể của học sinh — xem mục 5. KHÔNG được sinh bài kỹ năng nền chung chung không gắn root-cause.

2. Đa dạng về SỐ LIỆU và KÝ HIỆU giữa các bài:
   - PHẢI dùng ít nhất 2 biến khác nhau trong 3 bài (ví dụ: $x$, $y$, $a$, $t$, $m$). Không dùng chỉ một biến duy nhất cho cả nhóm.
   - PHẢI xen kẽ dấu: có bài hệ số dương, có bài hệ số âm, có bài hỗn hợp.
   - Hệ số phải khác nhau rõ rệt giữa các bài (không sinh hai bài similar liền nhau đều là cùng khuôn bình phương hoàn chỉnh nếu yêu cầu đa dạng dạng con).
   - Có thể thay đổi bậc (bậc 1, bậc 2, bậc 3 đơn giản) nếu chủ đề cho phép.

3. Độ khó:
   - Bài duy nhất trong "remedial" phải DỄ HƠN bài gốc (hệ số nhỏ, ít bước), tập trung một điểm yếu.
   - "similar" sắp theo thứ tự từ cùng độ khó → hơi khó hơn bài gốc một chút (bài 2 khó hoặc dài hơn bài 1 một chút nếu phù hợp).

4. Tự kiểm tra trước khi trả kết quả:
   - Nếu nhận ra 2 bài trong "similar" có CÙNG khuôn mẫu, PHẢI viết lại một bài sang dạng con khác.
   - Bài "remedial": nếu HS có nhiều root-cause (≥ 2), chọn ONE bài nhằm vào **một** root-cause — ưu tiên root-cause ảnh hưởng lớn nhất đến đúng/sai của bài; không cố nhét hai lỗi khác hẳn phạm trù vào cùng một bài làm lan man (nếu bắt buộc phân biệt, chọn một lỗi để luyện trong remedial và để các lỗi còn lại được "mistakes"/"nextSteps" nêu). Nếu HS chỉ 1 lỗi — remedial chỉ chữa lỗi đó. Nếu không lỗi — remedial là tiền đề sát nhất của bài gốc (xem 5.3).

5. NGUYÊN TẮC BÀI TẬP BỔ TRỢ ("remedial") — QUAN TRỌNG NHẤT, PHẢI TUÂN THỦ TUYỆT ĐỐI:

   Mục tiêu bài tập bổ trợ KHÔNG phải là "ôn lại kiến thức nền" chung chung, mà là GIÚP HỌC SINH SỬA ĐÚNG CHỖ MÌNH VỪA SAI (hoặc củng cố tiền đề sát khi không lỗi). Vì vậy **đúng 1 bài** "remedial" phải được suy ra từ PHÂN TÍCH LỖI của chính bài làm học sinh.
   ...
   5.1. Quy trình bắt buộc (thực hiện theo đúng thứ tự):
     ...
     (b) Bài "remedial" duy nhất là một bài ISOLATED: CHỈ kiểm tra đúng (một) kỹ năng/root-cause ưu tiên cao nhất trong các lỗi đã thấy, gỡ yếu tố thừa.
         Bài remedial phải DỄ HƠN rõ rệt bài gốc (hệ số nhỏ, ít bước hơn) để HS tập trung vào điểm yếu.
     (c) Chỉ có 1 slot: chọn một root-cause — nếu có ≥ 2 lỗi khác nhau, chọn cái **ảnh hưởng nhất** tới đáp án hoặc lập luận chính. Nếu chỉ 1 root-cause: remedial rèn đúng lỗi đó. Nếu không có "mistakes" (bài rất tốt): remedial là bài tiền đề gần nhất trong chủ đề (xem 5.3).

   5.2. Bài "remedial" PHẢI thoả cả 4 điều sau:
     - Rèn ĐÚNG MỘT kỹ năng/root-cause được chọn. Không được "combo" nhiều kỹ năng không liên quan trong cùng một bài (một chủ điểm mở rộng tự nhiên thì được, ví dụ khai triển rồi rút gọn cùng một biểu thức).
     - Dễ hơn bài gốc (hệ số nhỏ, bậc thấp hơn hoặc ít bước hơn).
     - Khi HS làm xong bài đó, HS thấy rõ một "chỗ cần sửa" được tách riêng. Nếu không đáp ứng tiêu chí này → viết lại.
     - Lời giải ("solution") phải NHẤN MẠNH đúng bước mà HS đã sai ở bài gốc (ví dụ: "(bước quan trọng: đổi dấu khi bỏ ngoặc có dấu trừ)"), để HS đối chiếu.

   5.3. Trường hợp đặc biệt:
     - Nhiều lỗi trong "mistakes": remedial chỉ chọn một dạng lỗi quan trọng nhất; các lỗi khác học sinh vẫn đọc trong "mistakes" và "nextSteps".
     - Học sinh chỉ có 1 lỗi duy nhất: remedial — luyện đúng lỗi đó ở mức rất dễ (gần bước hay sai trên bài làm).
     - Học sinh không có lỗi nào (score ≥ 9, "mistakes" rỗng): remedial — một bài tiền đề quan trọng nhất của bài gốc (cùng chủ đề, dễ hơn), không lan man.
     - Bài gốc chỉ yêu cầu tính toán cơ bản: remedial vẫn bám root-cause hoặc tiền đề đã chọn, dùng số/biến khác bài gốc để tránh copy.

   5.4. Ví dụ "ĐÚNG" — bài gốc: "Chứng minh $(2n+3)^2 - (2n+1)^2$ chia hết cho 8".
     Giả sử "mistakes" của HS gồm khai triển sai và thiếu kết luận chia hết — chọn ưu tiên chữa lỗi khai triển (ảnh hưởng trực tiếp tới chứng minh).
     "remedial" ĐÚNG (đúng 1 bài):
       * Một bài cố định: "Khai triển $(2x + 3)^2$" — chỉ kiểm tra bình phương một tổng, hệ số nhỏ; lời giải nhấn mạnh số hạng $2 \\cdot 2x \\cdot 3$.

   5.5. Ví dụ "SAI" cho cùng bài gốc trên (TUYỆT ĐỐI KHÔNG ĐƯỢC LÀM):
     "remedial" (lạc đề):
       * "Giải $2x + 3 = 7$"     // không liên quan lỗi khai triển / chứng minh chia hết
     LỖI: Không chữa root-cause trong "mistakes". → PHẢI VIẾT LẠI như mục 5.4.
...
LỖI: Cả 2 đều cùng khuôn $x^2 + bx + c$ dạng bình phương → thiếu đa dạng → BẮT BUỘC VIẾT LẠI.

VÍ DỤ ĐÚNG:
"similar" cho bài "phân tích $P = x^2 - 4x + 4$":
[
  { "problem": "$A = y^2 - 6y + 9$" },        // Bình phương (đổi biến)
  { "problem": "$B = 4a^2 - 9$" }             // Hiệu hai bình phương — khác dạng bài 1
]
- FORMAT CHÍNH XÁC: 
