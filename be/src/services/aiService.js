...
BẮT BUỘC SỐ LƯỢNG: "similar" phải có ĐÚNG 3 phần tử; "remedial" phải có ĐÚNG 2 phần tử — không thêm, không bớt.
...
Quy tắc:
1. Cả 3 bài trong **"similar"** PHẢI cùng **loại nhiệm vụ** với bài gốc (lập PT thì similar cũng là lập PT; giải PT thì similar cũng là giải PT; không đổi sang dạng khác).
2. **Nếu bài gốc là LẬP phương trình bậc nhất một ẩn từ lời văn:**
   - Mỗi "problem" PHẢI là một **bài toán lời văn mới** (bối cảnh khác nhau giữa 3 bài: tuổi, chuyển động, mua bán, tỉ lệ, hình học đơn giản…), đủ dữ kiện để học sinh **tự lập** phương trình.
...
1. Đa dạng TRONG CÙNG một nhóm ("similar" 3 bài, "remedial" 2 bài):
   - 3 bài trong "similar" PHẢI thuộc 3 DẠNG CON KHÁC NHAU của cùng chủ đề với bài gốc.
     Ví dụ nếu bài gốc là "phân tích đa thức thành nhân tử":
       ...
       * Bài 3: Tam thức bậc hai tách hạng tử ($x^2 + bx + c$ với $b, c$ khác hằng đẳng thức) HOẶC nhóm hạng tử / đặt nhân tử chung — chọn 3 dạng khác nhau, không trùng khuôn.
     Ví dụ nếu bài gốc là **lập phương trình bậc nhất một ẩn từ bài toán có lời văn**:
       * Ba bài similar là **ba bài lời văn khác nhau** (tuổi, chuyển động đều, mua bán, tỉ lệ, hình học đơn giản…), mỗi bài yêu cầu **lập** phương trình (có thể thêm "tìm ẩn" nếu đề gốc có).
       * TUYỆT ĐỐI KHÔNG lấy phương trình dạng "$ax+b=c$" sẵn để "giải" thay cho bài lời văn lập PT.
     TUYỆT ĐỐI KHÔNG được sinh 3 bài cùng một khuôn (ví dụ cả 3 bài đều là "bình phương một tổng/hiệu").
   - 2 bài trong "remedial" PHẢI bám SÁT lỗi cụ thể của học sinh — xem mục 5. KHÔNG được sinh bài kỹ năng nền chung chung không gắn root-cause.

2. Đa dạng về SỐ LIỆU và KÝ HIỆU giữa các bài:
   - PHẢI dùng ít nhất 2 biến khác nhau trong 5 bài (ví dụ: $x$, $y$, $a$, $t$, $m$). Không dùng chỉ một biến duy nhất cho cả nhóm.
...
4. Tự kiểm tra trước khi trả kết quả:
   ...
   - Nếu cả 3 bài trong "similar" đều cùng dạng con, PHẢI sinh lại toàn bộ nhóm.
   - Với đúng 2 slot "remedial": ưu tiên mỗi bài gắn một root-cause khác nhau nếu HS có từ 2 lỗi trở lên. Nếu HS chỉ có 1 lỗi duy nhất — xem mục 5.3 (bài 1 + bài 2 cùng trục chữa nhưng khác mức độ hoặc bài 2 là tiền đề gần).

5. NGUYÊN TẮC BÀI TẬP BỔ TRỢ ("remedial") — QUAN TRỌNG NHẤT, PHẢI TUÂN THỦ TUYỆT ĐỐI:

   Mục tiêu bài tập bổ trợ KHÔNG phải là "ôn lại kiến thức nền" chung chung, mà là GIÚP HỌC SINH SỬA ĐÚNG CHỖ MÌNH VỪA SAI. Vì vậy 2 bài "remedial" phải được suy ra từ PHÂN TÍCH LỖI của chính bài làm học sinh (hoặc tiền đề sát lỗi khi chỉ có một lỗi / không lỗi — mục 5.3).
   ...
   5.1. Quy trình bắt buộc (thực hiện theo đúng thứ tự):
     ...
     (b) Mỗi bài "remedial" là một bài ISOLATED: CHỈ kiểm tra đúng (một) kỹ năng/root-cause, gỡ yếu tố thừa.
         Bài remedial phải DỄ HƠN rõ rệt bài gốc (hệ số nhỏ, ít bước hơn) để HS tập trung vào điểm yếu.
     (c) Chỉ có 2 bài: nếu có ≥ 2 root-cause, gán bài 1 → root-cause nặng/quan trọng nhất, bài 2 → root-cause tiếp theo. Nếu chỉ 1 root-cause: bài 1 rèn lỗi đó (rất dễ), bài 2 cùng trục lỗi nhưng nâng một bước HOẶC tiền đề gần nhất (xem 5.3). Nếu > 2 root-cause, chỉ chọn 2 cái ảnh hưởng lớn nhất.
   ...
   5.3. Trường hợp đặc biệt:
     - Học sinh chỉ có 1 lỗi duy nhất: Bài remedial 1 — luyện đúng lỗi đó ở mức rất dễ; Bài remedial 2 — cùng trục lỗi nhưng nâng một bước (gần bài gốc hơn) HOẶC một bài "tiền đề gần nhất" giúp làm lại được bước hay sai. KHÔNG được hai bài copy nhau.
     - Học sinh không có lỗi nào (score ≥ 9, "mistakes" rỗng): Bài 1 — kỹ năng tiền đề quan trọng nhất của bài gốc; Bài 2 — tiền đề thứ hai hoặc một bước nâng nhẹ (vẫn trong chủ đề), không lan man.
     - Bài gốc chỉ yêu cầu tính toán cơ bản: 2 bài bổ trợ PHẢI đa dạng tình huống (số dương / âm, biến khác…) của đúng kỹ năng đó.
   ...
   5.4. Ví dụ "ĐÚNG" — bài gốc: "Chứng minh $(2n+3)^2 - (2n+1)^2$ chia hết cho 8".
     ...
     "remedial" ĐÚNG (đúng 2 bài):
       * Bài 1 (chữa lỗi 1, mức rất dễ): "Khai triển $(2x + 3)^2$" — chỉ kiểm tra bình phương một tổng, hệ số nhỏ; lời giải nhấn mạnh số hạng $2 \\cdot 2x \\cdot 3$.
       * Bài 2 (chữa lỗi 2): "Cho $A = 24k$. Chứng minh $A$ chia hết cho $8$" — chỉ kiểm tra kỹ năng kết luận "$A = 8 \\cdot \\text{số nguyên}$".

   5.5. Ví dụ "SAI" cho cùng bài gốc trên (TUYỆT ĐỐI KHÔNG ĐƯỢC LÀM):
     "remedial" (2 bài nhưng lạc đề):
       * "Tính $(x+1)^2$"       // quá chung chung, không gắn root-cause
       * "Giải $2x + 3 = 7$"     // không liên quan lỗi khai triển / kết luận chia hết
     LỖI: Không chữa đúng lỗi khai triển $(2n+3)^2$ hay lỗi không kết luận chia hết cho 8. → PHẢI VIẾT LẠI theo cách như mục 5.4.
...
LỖI: Cả 3 đều cùng khuôn $x^2 + bx + c$ dạng bình phương → thiếu đa dạng → BẮT BUỘC VIẾT LẠI.
