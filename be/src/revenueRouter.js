...
revenueRouter.get("/sheets/:reportDate", async (req, res) => {
  ...
  const conNoLedgerSafe =
    doc.conNoLedger != null ? normalizeConNoLedger(doc.conNoLedger, new Date()) : emptyConNoLedger();
  res.json({
...
revenueRouter.put("/sheets/:reportDate", async (req, res) => {
  ...
  const bodyConNoLedgerProvided = req.body != null && Array.isArray(req.body.conNoLedger);
