/**
 * Shared "Ocean UI" shell: slate/cyan gradients + grid + wave — same language as AssignmentsList.
 */
...
export default function OceanShell({
  ...
  return (
    <div
      className={`relative flex min-h-[100dvh] min-h-screen flex-1 flex-col overflow-hidden bg-slate-950 text-slate-100 ${className}`}
    >
    ...
    <div className="relative flex min-h-screen flex-col overflow-hidden bg-slate-950 text-slate-100">
      ...
      <div className="relative z-10 flex flex-1 items-center justify-center px-6">
        <div className="text-center">
          <div className="mx-auto mb-4 h-10 w-10 animate-spin rounded-full border border-cyan-300/40 border-t-transparent" />
          <p className="text-sm uppercase tracking-[0.3em] text-cyan-200/80">{message}</p>
        </div>
      </div>
    </div>
  );
...
export function OceanPageError({ title = 'Có lỗi xảy ra', message, onRetry, retryLabel = 'Thử lại' }) {
  return (
    <div className="relative flex min-h-screen flex-col overflow-hidden bg-slate-950 text-slate-100">
      ...
      <OceanWave />
      <div className="relative z-10 flex flex-1 items-center justify-center px-6">
        <div className="max-w-md rounded-3xl border border-rose-300/30 bg-slate-900/80 px-8 py-6 text-center shadow-2xl shadow-rose-950/40 backdrop-blur-xl">
          <p className="mb-3 text-sm uppercase tracking-[0.28em] text-rose-200/80">{title}</p>
          <p className="mb-5 text-slate-100">{message}</p>
          {onRetry ? (
            <button
              type="button"
              onClick={onRetry}
              className="inline-flex items-center justify-center rounded-2xl border border-cyan-300/40 bg-gradient-to-r from-cyan-500/80 to-blue-600/80 px-5 py-2.5 text-sm font-medium text-white shadow-lg shadow-cyan-950/40 transition hover:-translate-y-0.5"
            >
              {retryLabel}
            </button>
          ) : null}
